...
struct MatchStmt : Statement { ExprPtr value; std::vector<std::pair<ExprPtr, StmtPtr>> cases; StmtPtr defaultCase; MatchStmt(ExprPtr v, SourceLocation loc) : value(std::move(v)) { location = loc; } void accept(ASTVisitor& visitor) override; };
